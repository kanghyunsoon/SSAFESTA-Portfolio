...
## ⚡ v3 증분 (2026-08-25) — 컨설턴트 피드백 반영 (이 절이 v2·원안보다 우선한다)

- **신규 3건**: `S15P21A604-239` [FE] Booth 용도별 Layout 템플릿·프리셋(GitLab #93) ·
  `240` [BE] Booth 방문·체류·Coin 계측 이벤트 로깅(#94) · `241` [AI] Rule-based + LLM
  Booth 초안 생성 구조 확정(#95). 라벨 `consultant-feedback`, **스프린트 미편성(백로그)** —
  타 파트 스프린트 용량을 임의로 채우지 않는다. 편성은 월요일 플래닝에서 팀이 결정.
- **기존 6건 연계**: `164`·`206`·`235`·`236`·`237`·`238` 설명 끝에 `컨설턴트 피드백 연계` 절
  append (GitLab 이슈 + `docs/KHS/conference/08·09` 근거 문서 상호참조). 본문 무변경.
- 게임 파트 대응분(#89~#92)은 08-24 랩업에서 이미 등록돼 있어 **신규 생성하지 않았다** (중복 방지).
- 선후행 링크(235 blocks 236·237)도 이미 배선돼 있어 건너뛰었다.
- 실행기: `jira_feedback_sync.mjs` — 멱등(연계 절·동일 제목·기존 링크 검사), `--dry` 지원.
- 총계: 에픽 15 · 이슈 202 (v2 199 + 3).
