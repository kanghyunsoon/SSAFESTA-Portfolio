...
- `외부 설정`은 부스명, 간판, 로고, 테마, 대표색, 영업 상태만 편집한다.
- `내부 꾸미기`는 기능 오브젝트와 장식을 배치하는 Layout Editor다.
- 두 영역은 저장 데이터와 권한을 분리해 외부 간판 변경이 내부 Layout을 덮어쓰지 않게 한다.
...
- 외부 문 상호작용 시 `부스 내부로 이동하시겠습니까?` 확인 모달을 표시한다.
- 활성 임대가 아니거나 운영 중지 상태면 입장 버튼을 비활성화하고 이유를 표시한다.
- 내부에서는 항상 찾기 쉬운 `부스 나가기` 동작을 제공한다.
- 임대가 만료되면 `임대가 종료되어 외부로 이동합니다` 안내 후 안전한 외부 위치로 이동한다.
- 빈 외부 슬롯은 이전 임차인의 간판·로고·Layout을 노출하지 않고 기본 `임대 가능` 상태로 표시한다.
