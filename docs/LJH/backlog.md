...
**열린 내 MR 은 3건이다** — [!266](https://lab.ssafy.com/s15-metaverse-game-sub1/S15P21A604/-/merge_requests/266)(-91 게스트 AT, `56b398ac`) · [!267](https://lab.ssafy.com/s15-metaverse-game-sub1/S15P21A604/-/merge_requests/267)(-426 boot watchdog, `8b81d6dc`) · [!268](https://lab.ssafy.com/s15-metaverse-game-sub1/S15P21A604/-/merge_requests/268)(-427 런타임 base·manifest 해석, `fb76d156`). 09-05 생성, 리뷰어 gudtnslwkd, 머지 순서 !266 → !267 → !268. 머지는 실환경 검증(회원/게스트 진입·게스트 401 제거·boot watchdog·컨테이너 runtime-config 주입·실 WebGL asset resolve) 전이라도 코드 리뷰 기준으로 가능하되 `Closes` 는 그 검증 전까지 안 쓴다. 게임 파트의 `!259`(-421 canvas id)는 FE 리뷰 노트 게시(승인 가능 판정) — 머지는 게임 파트 몫. `!250`(World Interaction Integration, Unity 3건 `-343`·`-415`·`-414`)은 09-05 12:59 에 develop 에 머지됐고, 같은 라운드의 `-416`(Consultation, Unity 변경 0)은 `!251` 로 갈라 09-04 에 머지했다. `!262`(T-111 문서, `-247`)도 09-05 에 머지했다. 그 전 08-28 에 `!67`·`!63`·`!93`·`!94`·`!95`·`!97` 을 develop 에 머지했고, 08-27 오전에 `!46`·`!37`·`!42`·`!43`·`!52`·`!59`·`!60`·`!62` 를, part-target `!13`·`!20` 은 `front` 로 소진했다.
...
- [ ] **#128 FE 결함 3건 + #127 서빙 URL — MR !266·!267·!268 리뷰·머지 대기.** 남은 것: 머지(순서 !266 → !267 → !268), `!259` 머지(게임 파트), 인프라 compose 에 `PUBLIC_UNITY_BUILD_BASE` 주입(#127 에 요청함), 월요일 develop head 통합 실측 5종(회원/게스트 World 진입·게스트 401 제거·boot watchdog·runtime-config.js 컨테이너 주입·실 Unity WebGL asset resolve). **#128 스레드에는 회신하지 않았다** — 항목별 처리는 !259 노트·#127 코멘트·MR 3건 본문이 담고 있고, 게임 파트 직접 회신은 별도 승인 사안
