...
최종 갱신: 2026-08-20 — SC-002 해소(PR #15), 블록 3 FE·BE 몫 전부 완료, 잔여는 C-04(기획)뿐
...
- 리뷰: ①②③④+BE 검토칸 전부 완료. 제목 `FE 검토 완료 (C-04 기획 승인 대기)`로 정정 — 전체 "확정"은 아님
- 013a: Unity 소유로 축소 확정 — FE는 WebGL 호스트·Access Token 전달만
- Clarification: C-03 확정(고정 크기), C-05 확정(낙관적 잠금, 409 body 형식까지), C-06 보류, C-07 후순위, **C-04만 진짜 미결(기획 승인 대기)**
- SC-002: 006 C-01에서 역산한 관찰 기준으로 해소
...
- [x] **SC-002 해소** — ✅ **완료** (2026-08-20). "시간 목표"는 006 C-01(부스 구역 진입 시에만 조회, 폴링·푸시 없음) 구조에서 성립하지 않는다고 판정 — 진입 시점 관찰 기준으로 재작성("Publish 이후 진입 시 이전 공개본 노출 사례 0건"). 서버 반영 방식(동기/캐시)은 규정하지 않되 결과로 제약. Codex 5항목 독립 검증 전부 CONFIRMED 후 커밋. 부수: Edge Cases의 stale C-05 태그도 제거. `front` 커밋 `7c5f9ad` → `develop` PR [#15](https://github.com/kanghyunsoon/ssafesta/pull/15) 병합(`242b48b`)
- [x] **제목 갱신** — `리뷰 — FE 1차 검토 (확정 아님)` → `리뷰 — FE 검토 완료 (C-04 기획 승인 대기)`로 교체(`7c5f9ad`에 포함). **"확정"으로 안 쓴 이유**: C-04(콘텐츠 미연결 오브젝트 공개 차단 여부)가 기획 승인 대기로 여전히 미결 — 리뷰 절차 완료와 spec 전체 확정은 다르다
- [ ] C-05 잔여 — 요청 스키마 `version` 위치만 미결 (BE 착수 시 확정 예정)
- [ ] **C-04** — 유일하게 남은 진짜 미결. 기획 승인 필요(FE 의견: 막지 않고 경고)
...
**블록 0~2 전부 종결. 블록 3은 FE·BE가 할 수 있는 부분 전부 완료** — 남은 건 타 파트 결정 대기 2건(C-04 기획 승인, C-05 요청 스키마 `version` 위치 BE 착수 시)뿐. 블록 4(speckit 파이프라인)는 C-04가 닫혀야 spec 전체 확정 상태가 되므로 그 전까지는 착수해도 잠정 상태.
