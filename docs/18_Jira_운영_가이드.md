...
## 6-1. Jira 상태 — 누가 언제 옮기나 (2026-08-26 개정)

보드는 3상태(**해야 할 일 / 진행 중 / 완료**)다. 상태 전이는 전부 자동이다.

| 상태 | 누가 | 언제 |
|---|---|---|
| 해야 할 일 | 사람 (플래닝) | 스프린트 편성 시 |
| **진행 중** | **자동** | 작업 브랜치(`{type}/S15P21A604-N-…`) **최초 push** 시 — GitLab Webhook → Jira Automation. 반복 push 는 멱등 |
| **완료** | **자동** | 커밋에 `Closes S15P21A604-N` 을 넣고 그 커밋이 **`develop`** 에 도달할 때 — 내장 연동. **완료의 기준은 develop 이다** (main 은 최종 완성본 전용) |

### 지킬 것

- **상태를 손으로 옮기지 않는다.** 진행 중도, 완료도 자동이다.
- `Closes` 는 그 작업으로 이슈가 끝날 때만 쓴다 — 중간 커밋에 쓰면 develop 머지 때 미완료 이슈가 닫힌다.
- 파트 브랜치에 `Closes` 를 적어도 그 시점에는 전환되지 않는다. **develop 도달이 기준**이다.
