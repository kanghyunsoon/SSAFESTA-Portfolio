...
## 6-1. Jira 상태 — 누가 언제 옮기나 (2026-08-25 개정)

보드는 3상태(**해야 할 일 / 진행 중 / 완료**)다. GitLab 내장 Jira 연동이 켜진 뒤 역할이 이렇게 나뉜다.

| 상태 | 누가 | 언제 |
|---|---|---|
| 해야 할 일 | 사람 (플래닝) | 스프린트 편성 시 |
| **진행 중** | **사람 (담당자)** | **작업 착수 시 직접 옮긴다 — 자동화 없음** |
| **완료** | **자동** | 커밋에 `Closes S15P21A604-N` 을 넣고 그 커밋이 **`main` 에 도달**할 때 |

### 지킬 것

- **상태를 임의로 바꾸지 않는다.** 예외는 '진행 중' 하나뿐이다.
- '완료'를 손으로 옮기지 않는다 — `Closes` 로 처리한다. 손으로 옮기면 실제 반영 여부와 어긋난다.
- 파트 브랜치에 `Closes` 를 적어도 그 시점에는 전환되지 않는다. **`main` 도달이 기준**이다.
- 담당자 배정은 팀이 한다 (자동화 대상 아님).

### 왜 '진행 중'은 자동이 아닌가

내장 연동은 커밋 키워드 기반이라 "작업을 시작했다"는 신호가 없다. 이전 CI 설계는 브랜치 push 를
착수로 간주했지만, 브랜치를 미리 파 두는 경우가 많아 실제 착수 시점과 어긋났다.
**사람이 옮기는 편이 정확하다.**

상세 동작·설정은 `docs/jira-gitlab-workflow.md` §5 를 본다.
