...
| **013a** | avatar-customization | WORLD-04 | Unity(UI) + FE(호스트·인증) + BE(저장) | 001, 002 | **P0 승격 (2026-08-12 팀 결정)** — Sidekick 모듈 파츠 커스텀 + 실시간 동기화는 POC 완료(소급 spec). **2026-08-16 결정: 완성된 Unity `CharacterLobby`를 정식 커스터마이징 화면으로 쓰고 React 오버레이 교체 요구는 폐기** — `FE.md` 013a 에 반영 완료(#26). 신규 잔여: Spring 저장(`PUT /users/me/avatar`, 인코딩 문자열 `fa`), 재접속 복원, 멀티클라이언트 E2E |
