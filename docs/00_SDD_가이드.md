...
> 작성: 2026-08-12 (Unity 리드) | 갱신: 2026-08-13 — **Codex 기준**으로 정리
...
`git pull` → **저장소 루트에서** Codex 실행 → 끝이다.
...
| 항목 | 비고 |
|---|---|
| Codex CLI (또는 Claude Code) | **반드시 저장소 루트에서** 실행. 하위 폴더면 `.specify/`를 못 찾는다 |
| **bash** | 스크립트가 `.sh`라서 **Windows는 Git Bash 또는 WSL**에서 돌린다 |
...
| 도구 | 접두사 |
|---|---|
| **Codex** (기준) | `$speckit-plan` |
| Claude Code | `/speckit-plan` |

이름은 같고 접두사만 다르다. **결과물은 같은 `specs/`에 쌓인다** — 누가 작업해도 한 곳에 모인다.

> **Codex가 스킬을 못 찾으면** 이렇게 시키면 똑같이 동작한다:
...
| Codex가 speckit 스킬을 못 찾음 | §5-2 아래의 "SKILL.md를 읽고 실행" 방식으로 우회 |
