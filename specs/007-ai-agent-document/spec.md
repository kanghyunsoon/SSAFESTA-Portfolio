...
- **FR-016**: 문서 처리 작업은 **`JobStatus`(`QUEUED`/`RUNNING`/`SUCCEEDED`/`FAILED`/`CANCELLED`)** 를 가져야 한다.
...
- **FR-021**: P0 문서 처리는 인프로세스 Worker로 시작하며, 부하 실측 결과 필요할 때 SQS 도입을 검토한다. 서버 재시작 복구를 위한 상태 저장·재시도 계약은 [Issue #11](https://github.com/kanghyunsoon/ssafesta/issues/11)에서 별도로 확정한다.
...
| C-04 | 처리 큐를 도입하는가? | AI + Infra | **확정: P0 인프로세스 Worker, 부하 실측 후 SQS 검토. 재시작 복구 계약은 [Issue #11](https://github.com/kanghyunsoon/ssafesta/issues/11)에서 협의** |
...
- Q: C-01~C-07의 권장안과 C-03 변경안을 확정하는가? → A: C-03은 SHA-256 중복 방지·명시적 교체 변경안을 적용하고, 나머지는 표의 권장안을 확정한다. 서버 재시작 복구 저장 방식은 Issue #11 협의 결과를 후속 반영한다.
...
| 항목 | 내용 | 완료 |
|---|---|---|
| ① Clarification 답변 (C-01~C-07) | 전 항목 확정. C-04의 서버 재시작 복구 저장 방식만 Issue #11 협의 결과를 후속 반영 | ☑ |
| ② 틀렸거나 과한 요구사항 지적 | | ☑ |
| ③ 빠진 요구사항 추가 | | ☑ |
