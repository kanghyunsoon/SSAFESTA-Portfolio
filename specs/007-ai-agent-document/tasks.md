...
- [X] T004 [P] [AI] Alembic 환경과 `ai` 스키마 migration 경로를 `festa-ai/alembic.ini` 및 `festa-ai/migrations/env.py`에 구성한다
...
- [ ] T008 [BE] `develop` 기준 최신 Flyway migration인 V12 다음으로 `public.ai_agents`, `public.ai_documents`, `public.ai_document_chunks` 구조를 보완하고 `EXPIRED`, `failure_reason`, `content_sha256`, `storage_provider`, `storage_bucket`, `file_size_bytes`, `object_key`, `chunk_count`, `processed_at` 및 필요한 제약을 `backend/src/main/resources/db/migration/V13__ai_document_pipeline.sql`에 추가한다
- [ ] T009 [BE] `public.storage_reconciliation_log`(`run_id·document_id·object_key·source_provider·target_provider·expected/actual size·type·sha256·status·attempt_count·failure_reason·checked_at·resolved_at`, `UNIQUE(run_id, document_id)`)를 T008과 같은 Flyway 파일 또는 다음 사용 가능한 버전에 추가한다
...
- [ ] T012 [P] [AI] async SQLAlchemy engine과 migration/runtime role 연결을 `festa-ai/app/db/session.py`에 구현한다
- [ ] T013 [AI] `JobStatus`와 callback 필드(`callback_attempt_no`, `callback_next_retry_at`, `callback_delivered_at`, `callback_terminated_at`, `callback_terminal_code`)를 포함한 `ai.document_jobs` 모델 및 상태별 제약을 `festa-ai/app/db/models/document_job.py`에 구현한다
- [ ] T014 [AI] T013의 callback 필드를 포함한 `ai.document_jobs` 테이블, 활성 Job 부분 유니크 인덱스와 pickup·lease·미전달·미종료 callback 인덱스를 `festa-ai/migrations/versions/001_create_document_jobs.py`에 구현한다
- [ ] T015 [P] [AI] Spring 소유 Agent·Document와 문서별 `storage_provider + storage_bucket + object_key`를 읽기 전용으로 검증하는 repository를 `festa-ai/app/repositories/document_repository.py`에 구현한다
...
- [ ] T017 [P] [AI] `booth_id + agent_id + READY` 필터를 강제하고 청크 전체 교체를 지원하는 repository를 `festa-ai/app/repositories/chunk_repository.py`에 구현한다
...
- [ ] T032 [P] [US2] [AI] 세 OpenAPI 요청·응답 스키마를 `festa-ai/tests/contract/test_document_processing_api.py`, `festa-ai/tests/contract/test_spring_status_api.py`, `festa-ai/tests/contract/test_spring_storage_reconciliation_api.py`에서 검증한다. callback 404 원인 코드와 방향별 Service Token의 401 응답, reconcile 요청의 `runId + documentId`·상태 enum·검증 metadata 필드를 먼저 검증하고 실패를 확인한다
- [ ] T033 [P] [US2] [AI] 활성 Job 멱등성·`SKIP LOCKED` 단일 소유·상태 전이·DB 권한 테스트를 `festa-ai/tests/integration/test_document_job_repository.py`에 먼저 작성하고 실패를 확인한다
...
- [ ] T035 [P] [US2] [AI] 청크 전체 교체 원자성·청크 감소·중간 실패 rollback 테스트를 `festa-ai/tests/integration/test_chunk_replacement.py`에 먼저 작성하고 실패를 확인한다
...
- [ ] T037 [P] [US2] [AI] 문서별 R2/MinIO 읽기, 저장소·Embedding 일시 장애의 1·5·15분 유한 재시도, PDF 파싱·스캔 PDF 오류와 처리 시작·성공 commit 직전 `DISABLED → CANCELLED` 전환 테스트를 `festa-ai/tests/unit/test_document_processing_service.py`에 먼저 작성하고 실패를 확인한다
...
- [ ] T042 [P] [US2] [BE] `INTERNAL_SPRING_TO_AI_TOKENS` 첫 값을 Bearer로 부착하는 FastAPI 처리 요청 client와 202/401/409/422 처리를 `backend/src/main/java/com/example/ssafesta/internal/ai/AiDocumentProcessingClient.java`에 구현한다
...
- [ ] T049 [US2] [AI] 처리 시작과 성공 commit 직전에 문서 snapshot·`DISABLED` 상태·`sourceHash`를 검증하고, 문서별 `storageProvider + bucket + objectKey` 저장소에서 다운로드하며 비활성 문서는 `CANCELLED`로 종료하는 파싱·청킹·임베딩·원자적 청크 교체 파이프라인을 `festa-ai/app/services/document_processing_service.py`에 구현한다
- [ ] T050 [P] [US2] [AI] 멱등 처리 요청 및 내부 Job 상태 조회 endpoint를 `festa-ai/app/api/v1/documents.py`에 구현한다
...
- [ ] T053 [P] [US2] [AI] `INTERNAL_AI_TO_SPRING_TOKENS` 첫 값을 Bearer로 부착하는 `PROCESSING/READY/FAILED/DISABLED` callback client와 401·409 및 404 원인 코드(`JOB_NOT_REGISTERED`만 재시도 가능) 처리·사용자 오류 정제를 `festa-ai/app/services/spring_status_callback.py`에 구현한다
...
**Independent Test**: 문서 목록에 마지막 Spring 상태가 표시되고, AI 서비스 중단 중에도 조회가 가능하며, 삭제 직후 관련 Document·Chunk·Job이 남지 않고 문서별 Provider 원본 삭제가 재시도된다.
...
- [ ] T059 [P] [US3] [BE] 사용자 삭제와 `EXPIRED` 24시간 경과 시 검색 제외·Chunk/Job cascade·문서별 Provider `DeleteObject` 재시도 테스트를 `backend/src/test/java/com/example/ssafesta/ai/AiDocumentDeletionIntegrationTest.java`에 먼저 작성하고 실패를 확인한다
...
- [ ] T062 [US3] [BE] 소유권 검증 후 Document를 삭제해 FK cascade로 Chunk·Job을 정리하고 문서별 Provider·bucket·objectKey 삭제 작업을 기록하는 로직을 `backend/src/main/java/com/example/ssafesta/ai/AiDocumentDeletionService.java`에 구현한다
...
- [ ] T073 [AI] `specs/007-ai-agent-document/quickstart.md`의 migration, 성공 처리, 중복, 강제 종료, callback 복구·404 원인별 재시도/종료, 청크 교체, 임대 만료, stale 개정본, 내부 인증·회전, 수동 Provider 전환, 격리 검증을 순서대로 실행하고 결과를 `festa-ai/tests/validation/quickstart-results.md`에 기록한다
...
- 문서 삭제는 Spring DB의 FK cascade와 문서별 Provider 비동기 정리로 처리하며 Spring→FastAPI 삭제 API를 신설하지 않는다.
