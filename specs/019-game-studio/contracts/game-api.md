...
| ③ | **`docs/08` §18의 게임 코드 표** — ✅ **이 MR에서 정정. 결정 대상 아니고 검토만** | §18의 `GAME_*` **6행** 중 2개(`GAME_PROJECT_VALIDATION_FAILED`·`GAME_PORTAL_UNAVAILABLE`)가 **어디에서도 쓰이지 않는 이름**이었다. 실제는 `GAME_PROJECT_INVALID`(FE `apiErrorCopy` 소비)와 `CONFIG_NOT_FOUND`다. `GAME_DELETED`·`GAME_NOT_PUBLIC`·`GAME_FORBIDDEN`·`GAME_DRAFT_NOT_FOUND`는 아예 없었다. **6행 → 11행**으로 교체·보강했다 | §18은 전 파트가 읽는 코드 목록이다. 틀린 이름이 남아 있으면 다음 사람이 그 이름으로 분기를 만든다 (#59가 말한 복제 구조와 같은 뿌리) |
