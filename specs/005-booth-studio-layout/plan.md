...
> **C-04 진행 상태**: 콘텐츠 미연결 오브젝트의 공개 차단 여부(spec.md:170)는 기획 승인 대기 중이다.
> 이 plan은 **FE 기존 의견("막지 않고 경고")으로 PROVISIONAL 확정하고 진행**한다(사용자 결정, 2026-08-21).
> 기획이 반대로 결정해도 영향 범위는 `PublishDialog` 1개 컴포넌트로 격리되도록 설계했다 — research.md R-01 참조.
> spec 전체의 "확정"은 아니며, C-04가 뒤집히면 이 plan의 해당 부분만 갱신한다(헌법 30조 — PROVISIONAL 항목은 `docs/26`에 등록).
...
부스 소유자가 웹 2D 톱뷰 편집기에서 오브젝트(최대 12개, 헌법 22조)를 배치·연결하고 Draft로 저장·Publish하는 FE 기능이다. Layout JSON(3파트 공통 계약, 좌표 왕복 검증 통과 — `docs/LJH/verify/block1-roundtrip.md`)을 **생산하는 쪽**의 구현이다.

기술 접근: SVG 기반 미터 좌표계 캔버스(픽셀 환산을 코드에서 소멸시킴), `useReducer` 단일 EditorState, `@tanstack/react-query`로 서버 상태, `expectedRevision` 낙관적 잠금(`GET /draft` 재로드로 충돌 해소, 자동 병합 없음).

지배 제약: **신규 런타임 의존성 0** + Layout JSON 스키마 동결(FR-013, 헌법 24조 — 미지 필드는 BE가 `409 LAYOUT_VALIDATION_FAILED`로 거부) + BE는 이미 배포돼 있어 Mock은 로컬 개발 보조로 강등.
...
**Constraints**: Layout JSON 필드 추가·변경 금지(FR-013) — BE는 계약 외 필드를 전부 거부한다(#36). FE 검증은 UX용 빠른 검증이고 최종 검증은 서버(헌법 16조)

**Scale/Scope**: 페이지 1개(`/app/studio/:boothId`) + 컴포넌트 5~6개 + 순수 모듈 2개(`coords.ts`, `validate.ts`), Object Type 10종
...
| 16 — 클라이언트 불신 | FE 검증 6종은 빠른 UX 검증이며 서버 최종 검증을 대체하지 않는다. Route Guard도 서버 인가(403)를 대체하지 않는다 | ✅ 통과 |
...
**Post-Design 재확인 (Phase 1 이후)**: `data-model.md` 작성 후에도 위 판정 변동 없음. C-04 판정 로직을 `ObjectType` 데이터 테이블(연결 요건 열)로 옮겨, 기획 결정이 뒤집혀도 판정표 갱신만으로 흡수되게 했다 — 24·30조 조건부 통과 상태를 코드 구조로 최소화했다는 뜻이지, 완전 해소는 아니다.
...
│   │   └── PublishDialog.tsx              [신규] 연결 요건 미충족 경고 목록 — C-04 격리 지점
│   └── lib/
│       ├── coords.ts                      [신규] 화면축↔Z 부호 반전 단일 지점
│       └── validate.ts                    [신규] FE 검증 6종(순수 함수, ObjectType 판정표 참조)
├── entities/layout/
│   ├── types.ts                           [신규] docs/10 §7 타입 전사(정확 일치 필수 — 미지 필드 거부)
│   ├── objectTypes.ts                     [신규] Object Type 10종 판정표(연결 요건 열 — C-04 판정 원본)
