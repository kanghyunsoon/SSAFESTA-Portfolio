...
| C-04 미연결 공개 | **막지 않고 경고.** FR-007은 "알린다"이지 "막는다"가 아니다. 공개 시 미연결 목록 표시 후 진행, 방문자 쪽은 016 FR-009와 동일하게 오류가 아닌 안내 — ✅ 이후 [#45](https://github.com/kanghyunsoon/ssafesta/issues/45)로 이 방향 확정 (2026-08-21) |
| C-05 동시 편집 | **`version` 낙관적 잠금 + 409 제안.** 현재 `PUT /layouts/draft`에 충돌 감지 수단이 없어 마지막 저장이 앞 작업을 덮는다 — 요청 스키마 변경이라 BE 합의 필요 |
| C-06 템플릿 종수 | FE 결정 불가 — 기획. 편집기는 서버 제공 목록에서 고르는 구조로 만든다. **부스 크기가 확정돼야 FR-003 스냅 간격과 영역 이탈 검증값이 정해진다** — ✅ 이후 [#19](https://github.com/kanghyunsoon/ssafesta/issues/19)·[#45](https://github.com/kanghyunsoon/ssafesta/issues/45)로 확정(6×6×2.72m, 1종) |
| C-07 공개 이력 | **되돌리기 MVP 제외 의견.** 단 `version` 단조 증가는 유지해야 한다 (C-05가 의존) — BE 결정 대기 |

**상정 시점**: C-03·C-06은 다음 Layout 계약 회의에 FE가 안건으로 올린다. C-04는 기획에 개별 확인. C-05·C-07은 BE 착수 전까지 합의한다 — BE가 Draft API 스키마를 확정하는 시점이 마지막 기한이다. *(✅ 전 항목 확정 완료 — 현재 상태는 위 Clarifications 표가 정본)*
...
| ② FR-003 | 스냅 격자의 **단위·간격이 없다.** 좌표는 미터로 확정됐으나 간격은 부스 크기(C-06)에 종속된다. 설정값으로 두고 확정 후 주입 — ✅ 이후 확정: `SNAP_METERS=0.25` ([#45](https://github.com/kanghyunsoon/ssafesta/issues/45), 2026-08-21) |
...
| 대상 | 지적 |
|---|---|
| FR-002 (템플릿) | C-06이 미정이라 "기본 구성 선택"의 **실체가 없다.** 1차는 `template` 문자열 저장 + 화이트리스트 검증까지만 하고, 템플릿이 실제로 무엇을 배치하는지는 기획 확정 후 FE가 채운다 — ✅ C-06 이후 확정(1종, [#45](https://github.com/kanghyunsoon/ssafesta/issues/45)): 1차 범위 판단 그대로 유효 |
...
> **FE·BE 검토 완료** (2026-08-14 / 2026-08-20). 미결이던 C-04·C-06은 이후 [#45](https://github.com/kanghyunsoon/ssafesta/issues/45)로 확정 (2026-08-21) — 현재 상태는 위 Clarifications 표가 정본이다.
