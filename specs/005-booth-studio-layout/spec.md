...
| ~~C-05~~ | ~~Owner와 Staff의 동시 편집 정책은?~~ | — | ✅ **확정: `version` 낙관적 잠금 + 409.** BE 채택([Issue #5](https://github.com/kanghyunsoon/ssafesta/issues/5), 2026-08-20). 409 응답 body는 **`{code, message, requestId}`로 확정·구현 완료** (2026-08-20, `docs/08` §1.3 — 클라이언트는 `code`로만 분기). 요청 스키마 `version` 위치는 BE 착수 시 확정 |
