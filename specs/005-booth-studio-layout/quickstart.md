...
## §6. C-04 경고 — 서버 warnings 렌더링 (PROVISIONAL)
...
4. **판정 주체 확인** — mock에서 같은 항목을 `warnings` 대신 `errors`로 옮겨 응답하게 바꾸면, FE 코드 수정 없이 진행 버튼이 잠기는지 확인한다. 기획이 C-04를 차단으로 확정했을 때 실제로 일어날 일이며, 이 시나리오가 통과해야 FR-016 설계가 성립한다
