...
| 값 | 의미 |
|---|---|
| `null` | 외형을 한 번도 저장하지 않았다. **서버는 기본값을 만들어 넣지 않는다** — 프리셋 폴백은 클라이언트 책임이다 (FR-010·FR-015) |
| 문자열 | Unity `AvatarAppearance.Encode()` 산출물. 서버는 저장값을 정규화·변형하지 않는다. 다만 `fa|` 형식의 `i=` 8슬롯만 소유권 검사 목적으로 읽는다 (R-05, 2026-09-01 C-06 변경) |
