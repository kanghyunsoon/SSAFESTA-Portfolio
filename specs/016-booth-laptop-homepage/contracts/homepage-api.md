...
- 검증(필드가 있고 문자열일 때, 순서대로 첫 위반으로 거부): blank 금지 → 길이 ≤ **2048** → URI 파싱·절대 URI → scheme ∈ {`http`, `https`} → **`@`(userinfo) 금지** → host 존재 → **포트가 있으면 1~65535**. trim·정규화 없이 원문 그대로 저장한다.
  > **포트 규칙과 한글 도메인은 2026-08-28 spec 009 구현에서 추가됐다.** 검증기를 `common/HttpUrlValidator` 로 공용화하면서 009 의 URL 5 종과 같은 규칙을 타게 됐고, 016 도 함께 엄격해졌다 — `https://x.com:99999` 는 `java.net.URI` 가 받아주지만 연결이 안 되는 주소라 400 이다(거부 문장: `홈페이지 주소의 포트 번호가 올바르지 않습니다. (1~65535)`). 반대로 **한글 도메인은 이제 통과한다** — `IDN.toASCII` 로 판정하고 저장은 원문 그대로다. 기존 네 거부 문장은 바이트 단위로 그대로이고 통합 테스트 19 개가 그것을 지킨다.
