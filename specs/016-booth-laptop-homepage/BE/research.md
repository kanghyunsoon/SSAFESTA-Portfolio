...
> **갱신 (2026-08-27)** — R-01·R-02는 처음에 팀 미결 항목(C-01·C-02)에 대한 **BE 제안**으로 썼다. 둘 다 [#97](https://lab.ssafy.com/s15-metaverse-game-sub1/S15P21A604/-/issues/97)에서 **2026-08-26 확정**됐다(리드 확정 + FE 동의). 제안이 그대로 채택돼 결론은 바뀌지 않았고, 상태 표기와 하단 "팀 확정 게이트" 표만 결과로 갱신했다. **구현 착수 게이트는 해제됐다.**
> 확정 과정에서 **실측 오류 1건이 드러나 정정했다** — 하단 "부수 정리 후보" 참조.

---

## R-01. 저장 위치 = `booths.homepage_url` — Layout 계약은 건드리지 않는다 (C-01 ✅ 확정)
...
- **절차 (헌법 30조) — 완료**: C-01은 docs/26에 3파트 합동 미결로 등록돼 있었고 구현자가 임의 확정하지 않았다. #97로 올려 **2026-08-26 확정**받았다 — 리드 확정 + FE 동의, 제안대로 채택. FE는 `LayoutObject` 타입에 URL을 담을 자리가 구조적으로 없다는 근거를 추가로 붙였다(`entities/layout/types.ts:22-30` — 계약에 없는 필드를 보내면 서버가 `409`로 저장을 거부하므로 Layout 경로는 FE가 보내지도 못한다).

## R-02. 개수 = 부스당 **1개** (C-02 ✅ 확정)
...
- **절차 — 완료**: docs/26 행이 *"FR-001의 '여러 개 허용 여부'(C-02)가 미결이라 함께 정해야 함"*이라 명시해 C-01과 **함께** 물었고, #97에서 같이 확정됐다. FE 근거: 현재 분기가 `null` 하나로 "미등록/미공개"를 처리하도록 이미 짜여 있다(`LaptopOverlay.tsx:38-58`, FR-009).
...
- **절차 — 완료**: "공개 상태 = published layout 존재"라는 해석을 [계약](../contracts/homepage-api.md) §6에 확인 항목으로 올려 **FE 동의**를 받았다(#97, 이견 없음). 같은 회신에서 회차 필드명이 endpoint별로 다르다는 점이 리드 확정됐다 — Booth 상세는 `publishedLayoutVersion`, Layout Draft·Publish는 `publishedVersion`, **합치지 않는다**(계약 §3 주의 박스).
...
## R-10. `LAPTOP` 경고 = **재해석** (플래그 전환 아님) — 초안의 실측 오류 정정

- **Decision**: `LayoutObjectType.LAPTOP`의 `requiresConfig`는 **`true`로 유지**하고, `LayoutValidator`의 `CONFIG_NOT_LINKED` 판정 근거만 *"`configId`가 없다"* 에서 *"부스에 홈페이지 URL이 등록돼 있지 않다"* 로 바꾼다. 경고 코드·봉투는 그대로다(신규 rule 0). 표는 [계약](../contracts/homepage-api.md) §3-1.
- **Rationale**: C-01 확정으로 LAPTOP은 `configId`를 영원히 갖지 않으므로, 현행 조건이면 정상 등록된 부스에도 경고가 **상시 오탐**으로 뜬다. 끄는 대신 근거를 바꾸면 *"방문자가 클릭했는데 아무것도 안 뜬다"* 를 막는 경고의 원래 목적이 유지된다. FE도 같은 이유로 재해석을 선호했다 — *"`false`로 바꾸면 'URL 미등록 노트북' 경고가 통째로 사라진다"*(#97).
- **⚠️ 초안 실측 정정**: 이 항목은 처음에 *"`requiresConfig()`는 저장소 전체에서 `LayoutValidator` **한 곳에서만** 쓰이므로 `false`로 내려도 다른 검증에 영향이 없다"* 고 적혀 있었다. **틀렸다.** develop 기준 호출처는 2곳이다.
  ...
  `false`로 내렸다면 LAPTOP이 **시야 확보 검사에서 통째로 빠져**, 방문자가 앞에 설 자리가 없는 배치를 아무도 경고하지 않게 됐을 것이다. 재해석안은 플래그를 건드리지 않아 이 부작용이 없다. 라인 번호도 초안 인용(`151-152`·`168`)에서 이동했으니 구현 시 위 표를 기준으로 한다.
- **파생 — FE 몫 2건** (#97 통보): ① `LAPTOP`에 `configId`를 보내지 않는다(보내면 `CONFIG_UNVERIFIED`가 붙는다) ② `entities/layout/objectTypes.ts:36`의 `warnOnMissingConfig`를 `false`로 — 서버가 `configId` 기준 경고를 내지 않으므로 두면 FE만 없는 경고를 띄운다.
- **왜 지금 하는가**: `LayoutValidator.java:203-205` 주석이 *"공개를 막을지는 C-04, 정해지면 이 호출이 `addError`가 된다"* 로 남아 있다. C-04가 그 방향으로 확정되면 **노트북이 있는 모든 부스가 공개 불가**가 된다.

---


## 팀 확정 게이트 (헌법 30조) — ✅ 전부 닫힘 (#97, 2026-08-26)

BE가 확정할 수 없는 4건을 결정 선점 대신 **확정 절차로** 다뤘고, 전부 회신을 받았다. 결과 전문은 [계약](../contracts/homepage-api.md) §6에 있다.

| # | 항목 | 이 plan의 제안 | 확정 결과 |
|---|---|---|---|
| C-01 | URL 저장 위치 | `booths` 컬럼, Layout 불변 (R-01) | ✅ **제안대로 확정** — 리드 확정 + FE 동의 |
| C-02 | 부스당 개수 | 1개 (R-02) | ✅ **제안대로 확정** |
| FR-011 | C-01과 양립하는지 | C-01로 가면 *"계약 확장이 필요하다"* 가 성립하지 않는다 | ✅ **⑵ 채택** — C-01대로 가고 FR-011 문구를 고친다. 리드 확정문은 계약 §6. `spec.md` 반영은 **리드 몫**(BE가 손대지 않는다) |
| docs/26 | 미결 행의 전제 | FE 의견의 *"컬럼 신규 추가"* 와 실측이 다르다 — 컬럼은 V1부터 있다 | ✅ **FE(행 작성자)가 오기 자인·정정 담당.** stale 행 1건(`BOOTH_LAPTOP_INTERACT` 미검증 표기)도 함께 |

구현 자체는 전부 가산적(기존 컬럼 + 응답 필드 추가 + endpoint 신설)이라 확정 대기와 병행해도 잃는 폭이 작았고, 실제로 제안이 그대로 채택돼 되돌린 것이 없다.

> **머지 정책 (2026-08-25 정정 → 2026-08-27 해제)** — 이 절은 처음에 *"확정 전 back 머지 금지"* 로 적혀 있었다. 그 의도는 "제안이 확정처럼 읽히는 것"을 막는 것이었는데, 실제 결과는 **+443줄이 워크트리 전용 브랜치에만 남는 유실 위험**(T-117 계열)이었다. 그래서 *"문서는 `제안` 표기를 유지한 채 머지하고 구현 착수만 확정 후"* 로 바꿨다.
> **#97 종료로 그 게이트가 풀렸다** — 이제 `제안` 표기를 지우고 확정본으로 머지하며, 구현 착수도 열렸다.
> 남은 교훈은 그대로다: 확정이 이슈에서 닫혔는데 문서가 따라가지 않으면 **닫힌 결정이 미결로 되돌아가 보인다.** 이 갱신(2026-08-27)이 늦은 이유가 그것이고, MR !3이 닫힌 채 계약이 develop에 한 번도 도달하지 않았던 것과 같은 구멍이다.
