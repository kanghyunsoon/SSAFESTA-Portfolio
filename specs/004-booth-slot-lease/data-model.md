...
**조회 성능용 인덱스는 두지 않는다** — 슬롯이 12개라 전체 조회가 곧 최적이다. 위 목록은 전부 **무결성 제약**이다.

> V6·V7은 처음 설계에 없었다. 부하 테스트에서 **한 회원이 슬롯 전체에 동시 요청해 임대·부스를 슬롯 수만큼 만드는 것**을 발견하고 추가했다 (T-110, 당시 슬롯 7개). I-3과 I-5가 코드의 조회 한 줄에만 기대고 있었고, 그건 check-then-act라 경합에서 통과한다.
>
