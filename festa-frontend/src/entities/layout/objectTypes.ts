...
export const OBJECT_TYPE_INFO: Record<ObjectType, ObjectTypeInfo> = {
  AI_AGENT: { category: 'FUNCTIONAL', warnOnMissingConfig: true },
  VIDEO_SCREEN: { category: 'FUNCTIONAL', warnOnMissingConfig: true },
  PROJECT_PANEL: { category: 'FUNCTIONAL', warnOnMissingConfig: true },
  SURVEY_KIOSK: { category: 'FUNCTIONAL', warnOnMissingConfig: true },
  RECRUITMENT_BOARD: { category: 'UNDETERMINED', warnOnMissingConfig: false }, // 요건 자체가 spec Key Entities에 없음
  CONSULTATION_DESK: { category: 'FUNCTIONAL', warnOnMissingConfig: true },
  LAPTOP: { category: 'FUNCTIONAL', warnOnMissingConfig: true }, // 016에서 URL 계약으로 요건이 바뀔 수 있음
  LIKE_VOTE: { category: 'UNDETERMINED', warnOnMissingConfig: false }, // 부스 자체가 대상일 가능성 — 오브젝트별 연결 불명
  FURNITURE: { category: 'DECORATIVE', warnOnMissingConfig: false }, // assetCode는 외형 선택이지 콘텐츠 연결이 아님
  DECORATION: { category: 'DECORATIVE', warnOnMissingConfig: false },
};
