...
{
    /// <summary>
    /// 부스 안을 "영업 중" 으로 밝히는 조명.
    ///
    /// <para><b>배치는 씬이다 — 프리팹이 아니다.</b> <c>main.unity</c> 의 각 슬롯 아래에
    /// <c>BoothLight_FestivalSlot_01</c>~<c>12</c> 로 하나씩 놓여 있다. 부스가 사용자별로
    /// 교체돼도 따라오게 하려면 프리팹 쪽이 자연스럽지만, 지금은 씬 배치가 화면 검증을 통과한
    /// 상태이고 옮기려면 씬 광원 12개를 함께 빼야 한다 (2026-09-05 결정).</para>
    ///
    /// <para><b>그래서 프리팹에는 절대 넣지 마라.</b> 씬 광원을 그대로 둔 채 부스 프리팹에도
    /// 이 컴포넌트를 붙이면 부스마다 광원이 <b>두 개</b>가 된다. 아래
    /// <see cref="LimitExposureOnStaff"/> 는 <b>자기 광원 하나</b>의 세기/거리² 만 보므로
    /// 둘의 합계를 잡지 못한다 — 각각은 상한을 지키는데 직원 얼굴에 닿는 총량이 두 배가 되어
    /// -355 에서 고친 "얼굴이 하얗게 탄다" 가 그대로 재발한다. 실제로 프리팹 7개에 붙은 채
    /// 커밋 직전까지 갔다 (T-109).</para>
    ///
