...
> 마지막 갱신: **2026-08-24** — 08-08 기록은 그대로 두고, 그 뒤의 변화는 문서 끝
> 날짜별 "갱신" 절에 모았다. **ALB 전제는 폐기됐다** (#30).
...
## 2026-08-24 갱신 — 플레이어 스케일 규격·입구 확정·GitLab 이관 대기

- **플레이어 스케일 ×1.25** (기준: 11층 방 모델 1 m = 12.5 unit — 실측 인체 22.4 unit).
  아바타 목표 높이 22.375, CC 16.875/2.75, walk 45 / run 65, 점프 v 56 / g 149.33
  (체공 0.75초, 높이 1.05 m), 카메라 11.25~45. 프리팹 직렬화 + 스크립트 기본값 양쪽 반영.
  **부스 규격 계약(로컬 미터·앵커 40·boothId)은 불변.**
- **이동 중 점프 = 즉시 도약** (발구르기는 제자리 점프만, 0.18초). 애니메이터 speed 쌍:
  Launch 1.667 ↔ 0.18초, Air 0.844 ↔ 체공 0.75초.
- 복도 입구 확정(진홍 커튼 + 목재 인방), 야시장 잡동사니 배치·병합, Crates/Food 팩
  실사용분만 병합 후 벤더 삭제.
- **GitLab 이관 대기** — 문서·워킹트리 정리 완료. 병합 아님, `game` 브랜치 그대로 이동 예정.

---

## 2026-08-23 갱신 — 내부 부스·포털·BGM

- **내부 부스 공간 12실** (`@BoothInteriors`): 40 m 홀 × 22 m 천장, 방마다 `BoothSlot_{id}`
  앵커(스케일 40) + ExpoKit 셸(트러스 제외) + `BoothRuntime` 실통신 배선(loadOnStart 는
  Spring 부스별 데이터 전까지 off). **규격 계약: 레이아웃 좌표 = 부스 로컬 미터(불변),
  앵커 스케일 40 = 표현 배율.** boothId 1~12 = FestivalSlot = Interior = 백엔드 식별자.
- **F 상호작용**: 부스 실물 표면 1.5 m 판정, 키캡 프롬프트(화면 52%), 로컬 하이라이트 링.
  텔레포트는 client-authoritative (T-177 절차) — 원격에도 위치 동기화.
- **구역 BGM**: Morning(11층)/Circus(축제) 2D 동시재생 + 위치 크로스페이드(계곡형 —
  동시 가청 0 m). 내부 부스는 축제 35%.
- 함정 기록: T-190(보간 카메라 × 오클루전), T-191(64k 병합 × 스태틱배칭), T-194(TextMesh
  ZTest Always).
