...
## 필수: 문서 규칙 — 상태 서술은 교체, 덧붙이기 금지

- **덧붙이기(append) 절대 금지.** `대기 → ✅ 확정`, `— 이후 #NN로 확정`, 취소선 뒤 새 값 병기 등
  낡은 서술 위에 갱신을 쌓는 방식은 어떤 문서에서도 쓰지 않는다. 낡은 문장은 **최종 상태
  문장으로 다시 쓴다**(내용 누락 없이). 변경 이력은 Git/Issue/PR이 담당한다 — 문서 본문이
  이력을 흉내내지 않는다.
- **상태의 정본(SSOT)은 1곳.** spec의 C-xx 등 미결 항목 상태는 Clarifications 표에서만 관리한다.
  다른 섹션은 상태(`대기`/`보류`/`확정`)를 복제하지 않고 확정된 동작·정책만 기술한다.
- **상태 변경은 정본화 작업이다.** C-xx 등이 확정되면: 해당 ID로 **파일 전체 grep** →
  SSOT 외의 상태 복제를 최종 결정문으로 교체 → 전 occurrence 확인까지가 완료 조건이다.
  체크박스 하나 바꾸고 끝내지 않는다. (근거: spec 005 자기모순 사고, #45 / spec 007 동일 패턴, #51)
