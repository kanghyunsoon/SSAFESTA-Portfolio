...
**버전**: v1.3 | **비준일**: 2026-08-12 | **최종 개정**: 2026-09-01 (v1.3 — 11조 SSAFY 소셜 로그인 추가)
...
2. **실시간/영구 분리**: Unity Dedicated Server는 실시간 월드 상태(위치/스폰/존/외형 식별자)만 권위를 갖는다. Coin·Lease를 게임 서버가 변경하지 않는다.
...
5. **아바타도 데이터로 생성한다**: 외형은 **식별자만** 동기화하고 3D 표현은 각 클라이언트가 로컬 생성한다. 외형 오브젝트에 NetworkObject를 붙이지 않으며, 캐릭터당 NetworkObject는 하나다.
...
11. **로그인은 소셜 전용**: 자체 이메일·비밀번호 회원가입을 만들지 않는다. **Google / Kakao / SSAFY** 소셜 로그인과 **게스트 로그인**만 제공한다.
...
23. **아바타 외형은 ID 집합으로 표현한다.** 네트워크에는 **고정 크기 구조체**(작은 값들)만 전송하고,
    저장·API에는 짧은 직렬화 문자열을 쓰되 **저장 컬럼은 `TEXT`**(또는 넉넉한 가변 문자열)로 만든다.
    메시·텍스처·머티리얼을 네트워크로 보내지 않으며, 외형 데이터는 **Spawn 시점과 변경 시점에만** 전송한다.
    항목 식별자(itemId)는 배열 인덱스가 아니라 **안정적인 값**이어야 하며 재사용·재배치하지 않는다.
    *과거 계약서의 "29~32자"는 무효다.*
...
- 개발 지침은 `docs/00_SDD_가이드.md`와 파트별 브리프(`docs/sdd/parts/`)를 따른다.
...
**Version**: 1.3.0 | **Ratified**: 2026-08-12 | **Last Amended**: 2026-09-01
