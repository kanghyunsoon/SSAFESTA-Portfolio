...
assert_contains "${openapi}" "^[[:space:]]+type:[[:space:]]+\[string,[[:space:]]+'null'\]$" 'worldId must allow string or null'
assert_contains "${openapi}" '^[[:space:]]+enum:[[:space:]]+\[11F,[[:space:]]+null\]$' 'worldId must allow only 11F or null'
